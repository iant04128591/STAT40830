...
# All you need for a ggplot is (1) a data frame, (2)an aesthetic, and (3)a geom
...
ggplot(mpg, aes(x = displ, y = hwy)) + geom_point(aes(colour = drv))
...
# I want to create a histogram of engine size (displ) by transmission (trans). Fill in the blanks [A] and [B]:
 ggplot(mpg, aes(x = displ)) +
   geom_histogram(bins = 10) + facet_wrap( ~ trans)
...
#ggplot(mpg, aes(x = displ, y = hwy)) + geom_point(position = 'sunflower')
# with
...
p = ggplot(mpg, aes(x = displ, y = hwy, colour = drv)) + 
  geom_text(aes(label= drv)) + 
  geom_point(size=0) +
  guides(colour = guide_legend(override.aes = list(size=3,linetype=0))) + 
  scale_colour_brewer(name = "Drive Type", palette = "Blues") 
p
...
glob_temp = read.csv('http://data.giss.nasa.gov/gistemp/tabledata_v3/GLB.Ts+dSST.csv',
                     skip = 1,
                     colClasses = 'numeric',
                     na.strings = '***')
str(glob_temp)
...
# Grand challenge. Using the global temperature data can you re-create the 3rd graph on this page 
# exactly: 
# http://data.giss.nasa.gov/gistemp/graphs/? 
# The plot is titled 'Monthly Mean Global Surface Temperature' and the data are at: 
# http://data.giss.nasa.gov/gistemp/graphs/graph_data/Monthly_Mean_Global_Surface_Temperature/graph.csv
# Hints:
...
# 4) Post your code in the box, including the command to load the data in from the web 
#(i.e. the first line should contain read.csv with a URL). 
#A decent attempt gets 6/10. 
#Matching mine exactly gets 9/10. Improving on mine get 10/10. 
#Include comments to say what you've done